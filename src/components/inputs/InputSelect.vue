<template>
  <div class="relative w-72 md:w-80 lg:w-96 ">
    <label
        ...
        :class="disabled? '':'text-gray1'"
        :for="id" @click="showOptions()" >
      {{ label }}
    </label>
    <div
      @click="showOptions()"
      class="h-10 w-full  rounded-md text-sm px-4 font-medium outline-none drop-shadow-sm transition-all duration-200 ease-in-out flex items-center justify-between"
      :class="[
          disabled? 'bg-[#F7F7F7] text-gray1':'text-primary',
    ...
    >
      <p v-if="model && (model[property] || model[property] === 0)">
        {{ model[property] }}
      </p>
...
const props = defineProps({
  ...
  iterableKey: { type: String, required: true },
});
